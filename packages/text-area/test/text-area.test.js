...
import { TextArea } from '../src/vaadin-lit-text-area.js';

const runTests = (tag) => {
  let textArea;

  beforeEach(async () => {
    textArea = fixtureSync(`<${tag}></${tag}>`);
    await nextRender();
  });
...
describe('TextArea + Polymer', () => {
  runTests('vaadin-text-area');
});

describe('TextArea + Lit', () => {
  const LIT_TAG = 'vaadin-lit-text-area';

  customElements.define(
    LIT_TAG,
    class extends TextArea {
      static get is() {
        return LIT_TAG;
      }
    },
